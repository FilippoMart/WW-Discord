...
- `games(play PK, modalita)` — play id format is `YY-MM-DD_n` (e.g. `26-07-14_1`), chosen so lexicographic sort = chronological sort. `_n` increments for multiple games the same day. `modalita` is `'Discord'` or `'Live'` (in-person) — all games before `26-08-28_1` are Discord (backfilled), ask the user which one for anything after that if not stated. Shown in Composizione partita.
